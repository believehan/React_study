...
/* 
    리액트에서는 링크를 걸기 위해 a 태그 대신 BrowserRouter 라이브러리의 Link 태그를 활용.
    실제 브라우저 렌더링 과정에서 Link 태그를 a 태그로 변환하여 처리.
*/
export default function ContentLists({ userList, getLoadList }) {
    /* 
        Link 태그에 대한 부모인 li 태그로 이벤트 위임 처리.
        Link 태그는 브라우저 렌더링 과정에서 a 태그로 변환되어 처리되므로
        비교시에도 LINK 가 아닌 A 로 비교해야함에 주의.
    */
