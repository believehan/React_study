...
export default function SearchInput({ users }) {
    ...
    - 라우터를 통해 다른 경로로 이동하는 데 사용되는 Hook 로써 useNavigate
      ...
      하여 해당 컴포넌트를 렌더링 가능.
      즉, useNavigate()는 Link 컴포넌트 없이도 프로그래밍 방식으로 라우팅하여
      해당 컴포넌트나 페이지로 이동을 가능하게 하는 Hook.
    ...
    function serchId(e) {
        ...
        if (users.find(user => user.userId === userId)) {
            ...
            - 쿼리 스트링은 URL의 끝에 ? 구분자를 기준으로 우변에 "키=값" 형식의
              쌍으로 이루어진 데이터를 표현함으로써 URL 에 간략한 추가 정보를
              전달하는 목적으로 사용.
              두 개 이상의 데이터를 전달할 때에는 & 이용하여 데이터를 구분하여 전송.

              ex)
                      http://localhost:8080/test4?doc=firstDocument&value=10
